function [] = SMC_n_vF(data,maxiterinc,savepathincome)
...
cd('/home/jdlight/ABBL_PMCMC/JOE_codes/Bootstrap/Inc/')
...
maxiter=maxiterinc; % # Stochastic EM iterations
rng('shuffle') % Generate seed
Ntau=11; % Set grid
Vectau=(1/(Ntau+1):1/(Ntau+1):Ntau/(Ntau+1))';

% try saving the file early (so if it fails we find out asap)
save(savepathincome, 'T','Ntau', 'Vectau')
...
% % pre-load vars to speed things up
res = load ('/home/jdlight/ABBL_PMCMC/JOE_codes/Results/211015_smc_n.mat',...
    'Resqfinal','Resqfinal_eps','Resqfinal_e0',...
    'OLSnew','OLSnew_e0','OLSnew_eps',...
    'veta1','veta','veps',...
    'b1','bL','b1_e0','bL_e0','b1_eps','bL_eps');

Resqinit = res.Resqfinal;
Resqinit_e0 = res.Resqfinal_e0;
Resqinit_eps = res.Resqfinal_eps;
OLSnew = res.OLSnew;
OLSnew_e0 = res.OLSnew_e0;
OLSnew_eps = res.OLSnew_eps;
veta = res.veta;
veta1 = res.veta1;
veps = res.veps;
b1 = res.b1;
...
%           
    ...
    disp('Income Iteration:')
    disp(iter)
    disp('Average persistence:')
    disp(mean(Mat*Resqinit))
...
save(savepathincome, 'T','Ntau', 'Vectau',...
    ...
    'b1', 'bL', 'b1_e0', 'bL_e0', 'b1_eps', 'bL_eps',...
    'OLSnew', 'OLSnew_e0', 'OLSnew_eps', 'veps', 'veta', 'veta1','mat_lik',...
    'stdY','meanY','meanAGE','stdAGE')
