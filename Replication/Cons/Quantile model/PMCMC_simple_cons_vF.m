...
% The first tends to be faster

% Method1: arrayfun
[test1,test2] = ndgrid(0:3);
uc=[test1(:),test2(:)];
uc= uc(uc(:,1)<=M1,:);
uc= uc(uc(:,2)<=M2,:);
Vect_AGE_tot=arrayfun(@(x) hermite(x,(AGE_tot-meanAGE)/stdAGE),uc(:,2),'Uniform',0);
Vect_A_tot=arrayfun(@(x) hermite(x,(A_tot-meanA)/stdA),uc(:,1),'Uniform',0);
MatClag1 = cat(2,Vect_A_tot{:}).*cat(2,Vect_AGE_tot{:});

% Method2: hermite loop
 MatClag2=[];
 for kk1=0:M1
     for kk2=0:M2
         MatClag2=[MatClag2 hermite(kk1,(AGE_tot-meanAGE)/stdAGE)...
             .*hermite(kk2,(A_tot-meanA)/stdA)];
     end
 end
