function [Resqinit_cons,Resqinit_a,Resqinit_xi,Resqinit_a1,OLSnew_cons,OLSnew_a,OLSnew_xi,OLSnew_a1,vcons,va,vxi,va1,b1_c,bL_c,b1_a,bL_a,b1_a1,bL_a1,b1_xi,bL_xi]=init_cons_pmcmc_corr()
...
global N K1 K2 K3 K3t K4 K5 Ntau Vectau tau ...
       ...
       Vect_YB Vect_ED YB EDUC meanEDUC stdEDUC meanYB stdYB
...
for iii=1:N
    ...
    Vect_Matdraw_tot=arrayfun(@(x) hermite(x,(Matdraw(iii,ENT(iii):EXT(iii))'-meanY)/stdY),uc(:,2),'Uniform',0);
    Vect_Eps_tot=arrayfun(@(x) hermite(x,(Y(iii,ENT(iii):EXT(iii))'-Matdraw(iii,ENT(iii):EXT(iii))' - meanY)/stdY),uc(:,3),'Uniform',0);
    MatClag = cat(2,Vect_A_tot{:}).*cat(2,Vect_Matdraw_tot{:}).*cat(2,Vect_Eps_tot{:}).*cat(2,Vect_AGE_tot{:}); 
